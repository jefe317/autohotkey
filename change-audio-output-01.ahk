; toggle audio out between "Speakers" and "Headphones"
...
; Check the current default device and toggle it
currentDevice := GetCurrentDefaultEndpoint()
if (currentDevice = GetDeviceID(Devices, "Speakers")) {
...
GetCurrentDefaultEndpoint()
{
    ; Get the current default audio endpoint
    IMMDeviceEnumerator := ComObjCreate("{BCDE0395-E52F-467C-8E3D-C4579291692E}", "{A95664D2-9614-4F35-A746-DE8DB63617E6}")
    DllCall(NumGet(NumGet(IMMDeviceEnumerator+0)+4*A_PtrSize), "UPtr", IMMDeviceEnumerator, "UInt", 0, "UInt", 0x1, "UPtrP", IMMDevice, "UInt")
    DllCall(NumGet(NumGet(IMMDevice+0)+5*A_PtrSize), "UPtr", IMMDevice, "UPtrP", pBuffer, "UInt")
